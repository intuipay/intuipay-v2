  ...
  const handleConnect = async (event: FormEvent<HTMLFormElement>) => {
    ...
    >
      ...
      <div className="space-y-2">
        <Label className="text-sm text-gray-600">Selected Wallet</Label>
        <div className="flex items-center p-3 gap-3 border rounded-lg bg-gray-50">
          {selectedWallet && (
            <>
              <Image
      ...
      <div className="space-y-2">
        <Label className="text-sm text-gray-600">Selected Network</Label>
        <div className="flex items-center p-3 gap-3 border rounded-lg bg-gray-50">
          <Image
            src={`/images/logo/${Networks.find(n => n.value === network)?.icon}.svg`}
            width={24}
            height={24}
            className="size-6"
            alt={Networks.find(n => n.value === network)?.label || ''}
            loading="lazy"
          />
          <span className="font-medium">
